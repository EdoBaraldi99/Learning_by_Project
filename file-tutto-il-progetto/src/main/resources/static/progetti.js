(() => {
     ...
     */
    function normalizeProject(raw, teamCountByProject, teamLeaderByProject, memberNamesByProject) {
        ...
        return {
            ...
            memberNames: memberNamesByProject.get(raw.idProgetto) || [],
    ...
    async function apiGetProjects(query = '') {
        ...
        const memberNamesByProject = new Map();
        let associati = [];
        if (associatiRes.ok) {
            ...
            associati.forEach(a => {
                ...
                if (a.dipendente) {
                    ...
                    if (!memberNamesByProject.has(id)) memberNamesByProject.set(id, []);
                    memberNamesByProject.get(id).push(name);
                }
            });
        }
        ...
        const projects = rawProjects.map(p => normalizeProject(p, teamCountByProject, teamLeaderByProject, memberNamesByProject));
    ...
    async function onSubmitCreateForm(e) {
        ...
        if (payload.dataInizio && payload.dataFine && payload.dataInizio > payload.dataFine) {
            errorEl.textContent = I18N.t('progetti.dateRangeError');
            errorEl.hidden = false;
            Toast.error(I18N.t('progetti.dateRangeError'));
            return;
        }
